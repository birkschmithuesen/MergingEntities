...
public:
    ...
    void update_accel_gyro() {
        ...
        a[0] = ((float)raw_acc_gyro_data[0] * acc_resolution) - acc_bias[0]/(float)CALIB_ACCEL_SENSITIVITY;  // get actual g value, this depends on scale being set
        a[1] = ((float)raw_acc_gyro_data[1] * acc_resolution) - acc_bias[1]/(float)CALIB_ACCEL_SENSITIVITY;
        a[2] = ((float)raw_acc_gyro_data[2] * acc_resolution) - acc_bias[2]/(float)CALIB_ACCEL_SENSITIVITY;

        temperature_count = raw_acc_gyro_data[3];                  // Read the adc values
        temperature = ((float)temperature_count) / 333.87 + 21.0;  // Temperature in degrees Centigrade

        // Calculate the gyro value into actual degrees per second
        g[0] = ((float)raw_acc_gyro_data[4] * gyro_resolution) - gyro_bias[0]/(float)CALIB_GYRO_SENSITIVITY;  // get actual gyro value, this depends on scale being set
        g[1] = ((float)raw_acc_gyro_data[5] * gyro_resolution) - gyro_bias[1]/(float)CALIB_GYRO_SENSITIVITY;
        g[2] = ((float)raw_acc_gyro_data[6] * gyro_resolution) - gyro_bias[2]/(float)CALIB_GYRO_SENSITIVITY;
    }
