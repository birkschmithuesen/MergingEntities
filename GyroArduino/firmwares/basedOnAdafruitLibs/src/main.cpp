...
unsigned long calibButtonTimeout = 1000 * 30; // calibration button will only work if pressed in the first x seconds (value in millis)
unsigned long statusReportInterval = 16;        // send reports every n loop cycles
unsigned long statusReportIntervalCounter = 0;  // current Cycle
...
void loop()
{
    ...
    //signal if there was a problem with the sensors by lighting the blue LED
    digitalWrite(PIN_BLUELED, imuCollection.highestErrorCode!=0);
...
// support for interactive calibration
    ...
    if (UserInterface::getCalibrationButtonState())
    {
        if (millis() < calibButtonTimeout)
        {
            calibrationManager.calibrateSequence();
        }
        else
        {
            calibrationManager.printCalibrations();
        }
    }
#endif
