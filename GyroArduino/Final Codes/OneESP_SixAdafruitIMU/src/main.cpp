...
void setup() {
  ...
  //Button not pushed -> in any case here : we read the stored calibration data and calibrate
    ...
    for(int i=0; i<nbrMpu; i++){
      ...
      cal[i].gyro_zerorate[0] = float(preferences.getChar("gyrobiasX", 0.0001))/100;
      cal[i].gyro_zerorate[1] = float(preferences.getChar("gyrobiasY", 0.0001))/100;
      cal[i].gyro_zerorate[2] = float(preferences.getChar("gyrobiasZ", 0.0001))/100;
